...
def 문단_내어쓰기_적용(문단_시작위치, text, 폰트크기_pt=None, 폰트=None, 굵게=None):
    ...
    ParagraphShapeIndentAtCaret는 문단 모양만 변경한다. 내어쓰기 뒤 문단
    전체 CharShape를 다시 적용하면 혼합 서식의 Bold가 대표값으로 평탄화될
    수 있으므로 글자 모양 복원 작업은 하지 않는다.
    """
...
def 표_헤더서식_전체_적용():
    ...
    로그(f"표 헤더/본문 서식 적용 완료 (검사한 컨트롤 영역 {방문영역수}개 / 적용된 표 셀 {셀수}개 / 한 칸 표 제외 {제외수}개 / 대상 아님·실패 {실패수}건)")
    return True
...
def 컨트롤_내부_자간조정():
    ...
    while True:
        ...
        if hwp.GetPos()[0] == 0:
            break
        if area in 한칸표영역:
            진단로그(f"[한 칸 표 제외] 영역 {area}: 자간·공백 서식 보존")
            continue
        while True:
            if 중단_요청됨():
                return False
