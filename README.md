...
여백, 대표 본문·제목·기호 문단의 글꼴/크기/굵기, 장평·자간·줄간격·문단 모양과 첫 표의
머리글/본문 서식을 분석하고, 사용자 프로필로 저장한 뒤 `서식 정리` 작업에 바로 선택합니다.
세부 설정의 **서식 복사하기**를 이용하면 저장 이름을 직접 지정할 수 있습니다.

서식 프로필은 `%APPDATA%\HwpAutoDocFit\formats`에 JSON으로 저장됩니다. Markdown은 HWP의
정밀 서식 속성을 표현할 표준 문법이 없어 프로필 저장 형식으로 사용하지 않습니다. JSON은
숫자 단위와 HWP 전용 속성을 손실 없이 저장하면서 이전 버전 프로필과도 호환됩니다.
