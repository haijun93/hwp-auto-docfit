...
한 줄로 뭉쳐진 문장은 다시 자연스러운 문단으로 이어 붙입니다. `\*\*굵게\*\*`처럼 백슬래시로
이스케이프된 강조 기호도 먼저 풀어낸 뒤 정리하고, 문장 끝에 붙는 `[1]`·`[2][4]` 같은 숫자
전용 각주 표시는 제거하되 `[별표 21의2]`처럼 글자가 섞인 대괄호는 그대로 남깁니다. HWP/HWPX
파일이나 한글 프로그램은 전혀 사용하지 않는 순수 텍스트 변환이며, **결과 복사**로 정리된
텍스트를 클립보드에 담아 원하는 곳에 붙여넣을 수 있습니다.
