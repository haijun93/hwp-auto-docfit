...
실행 파일은 `hwp-auto-docfit.py`이며, 프로그램 버전은 `1.67 Beta 1`입니다.
...
복사 전 확인 창에는 제목·중제목·소제목·본문·내용·부연설명의 계층과 함께
...
점(dot) 모양의 문두기호(·, ‧, ∙, ㆍ 등)는 모두 대표 기호 **•**로 통일해서 표시·적용합니다.
기본 서식의 좌우 여백은 중앙부처 관행값인 20mm입니다(서식 복사로 만든 프로필은 원본
문서에서 추출한 여백 값을 그대로 사용).
연도 표기 앞의 작은따옴표류(', ‘, ʼ, ′, `, ´)는 문서 전체에서 대표 기호 **’**로
통일합니다(예: `'21년` → `’21년`).
**문단 위 여백**에는 참고(*, ※) 등 더 깊은 위계를 거쳐 상위 기호(□/ㅇ)로 되돌아올 때
그 기호의 기본 간격을 확대해 새 항목의 시작임을 드러내는 **복귀 시 확대(%)** 옵션이
있습니다(기본 150%, 100%면 항상 기본 간격만 사용).
