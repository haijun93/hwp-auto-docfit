...
- 파일·폴더 드래그 앤 드롭과 여러 문서 일괄 처리, '텍스트 붙여넣기'로 파일 없이도 텍스트를 .txt로 저장해 목록에 바로 추가
...
- 붙여넣은 텍스트 정리: 생성형 AI 채팅창(제미나이·클로드·챗GPT 등)에서 복사한 답변의 마크다운 기호를 지우고 흐트러진 띄어쓰기·줄바꿈을 복원. '개조식으로 변환'은 제목·글머리 기호의 계층 구조를 ㅁ/ㅇ/-/• 문두기호로 바꿈
